...
Post-hoc explanations are generated after a model has produced its output and aim to explain that output by analysing the relationship between inputs and predictions. Rather than modifying the model itself, these methods treat the model as a fixed system and apply external analysis techniques to interpret its behaviour. Within this category, several sub-families can be distinguished. 
