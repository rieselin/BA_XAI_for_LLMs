...
It should be noted that KernelExplainer computes a model-agnostic approximation of Shapley values rather than exact values. Exact computation would require evaluating all possible feature subsets, which is combinatorially intractable for prompts of any practical length. The approximation quality is controlled by the number of masking samples, a trade-off discussed in Section~\ref{sec:empUserStudy}. This is a deliberate methodological choice that prioritises feasibility over exactness, and is standard practice when applying SHAP to high-dimensional or black-box models.

Words were annotated by section (system prompt, instruction, or input) and rendered in the frontend with colour-coded highlighting: orange shades for words that promoted the response and blue shades for words that suppressed it, with opacity scaled to the magnitude of each word's SHAP value. Bar charts were generated server-side using Matplotlib and returned as base64-encoded PNG images, one for each prompt section and one overall. A second inference call to the instruct model then produced a plain-language explanation of why the top-scoring words likely had the influence they did, referred to in the frontend as the AI counter-analysis.
...
In the frontend, each token was rendered inline with a background colour reflecting its confidence tier: green for high confidence (above 75\%), amber for medium (45–75\%), and red for low confidence (below 45\%). An animated ring indicator summarised the mean confidence across the full response.
