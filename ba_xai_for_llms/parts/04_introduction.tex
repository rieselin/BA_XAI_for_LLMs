...
\begin{quote}
\textit{``Is it [the GenAI Model's output] not only correct, but actually, can I trust it?''}\\
...
Federal Chancellery found that 96~percent of the Swiss population had heard of
LLM-based tools such as ChatGPT, Gemini, or Co-Pilot \cite{bern_chance_2025}.
Of those familiar with these applications, 59~percent reported using them at least
occasionally, and 59~percent of users also employ them in a professional context
\cite{bern_chance_2025}.
These figures illustrate that LLMs are no longer a niche technology: they are embedded
...
model errors first-hand may develop excessive skepticism that prevents them from
...
XAI encompasses methods and techniques designed to make AI behavior more
...
human-centered techniques that leverage LLMs' own language generation capabilities
...
Post-hoc attribution methods, most notably SHAP \cite{lundberg_unified_2017} and LIME,
...
Intrinsic interpretability methods embed transparency into the model's own generation process.
...
Both ChatGPT \cite{noauthor_chatgpt_nodate} and Claude \cite{noauthor_claude_nodate} surface step-by-step reasoning traces as a standard
transparency mechanism, making CoT the most widely deployed explanation paradigm
...
Human-centered and narrative explanations represent a third strand, in which
LLMs generate natural language accounts of their own behavior, either through
...
Human-AI Collaboration (HAIC) research has drawn attention to additional dimensions
...
\begin{itemize}
    ...
    \item \hyperref[ch:discussion]{Chapter 5} discusses the findings, limitations, and future research directions.
\end{itemize}