...
Across all XAI conditions, trust was lower than the plain LLM baseline ($\text{Mdn} = 4.0$), which produced the highest and most stable trust ratings of any condition. At first glance, this might suggest that XAI methods are counterproductive. However, this interpretation is too hasty. The baseline condition presented participants with fluent, confident outputs and no further information, precisely the conditions under which miscalibrated over-trust is most likely to develop. As discussed in Section~\ref{sec:trust_in_context_of_llms}, LLMs are susceptible to hallucination, bias, neglect of relevant factors and dishonesty, and their outputs are delivered with the same confident fluency regardless of accuracy. Users who default to high trust in the absence of any explanatory signal are not necessarily better calibrated; they are simply more credulous.
...
The inferential results support this interpretation. A Friedman test across all five
conditions was highly significant ($\chi^2(4) = 110.83$, $p < .001$), and pairwise Wilcoxon
tests with Holm correction confirmed that LLM baseline trust was significantly higher than
CoT, SHAP, and CONF ($p < .001$ in each case), but \emph{not} significantly higher than
the EXPL condition ($p = .097$). The non-significant gap between LLM and EXPL is
noteworthy: a plain conceptual explanation of how LLMs work produces trust levels
statistically indistinguishable from the uninformed baseline, suggesting that general
understanding, rather than reduced credulity, accounts for the trust outcome in that condition.
...
This thesis set out to investigate whether and how Explainable AI methods can help lay users calibrate their trust in LLM outputs. The empirical user study with $n = 175$ prior LLM users found that all three XAI methods (Chain-of-Thought prompting, SHAP-based attribution with counterfactual reasoning, and confidence indication) reduced trust relative to a plain LLM baseline. Chain-of-Thought emerged as the most preferred and most trust-disrupting method, confidence indication as an efficient and moderately preferred alternative, and SHAP as technically grounded but poorly suited to non-technical audiences. A general explanation of how LLMs work produced competitive trust outcomes, underscoring the value of conceptual framing alongside per-output explanation.

The trust ordering observed descriptively (LLM $>$ EXPL $>$ SHAP $>$ CONF
$\approx$ CoT) was confirmed by a Friedman test ($\chi^2(4) = 110.83$, $p < .001$)
and Holm-corrected Wilcoxon follow-up tests, with the sole non-significant gap between
the LLM baseline and EXPL underscoring the practical value of conceptual framing as a
complement to per-output explanation methods. Building on these findings, a prototype was
developed that combines Chain-of-Thought reasoning with per-step confidence scores and
automatic regeneration of low-confidence steps. This combination targets the central design
gap identified in the literature and the study results: explanation methods that are
technically grounded enough to be reliable, yet rendered in forms accessible enough to be
useful for general users. Together, the empirical findings and the prototype contribute a
set of concrete design recommendations for LLM-based applications in which appropriate
user trust is not merely desirable but, in high-stakes domains, essential.